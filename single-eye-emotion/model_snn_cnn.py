...
class SNNCNN3(nn.Module):
    ...
    def forward(self, frame, event):
        ...
        for step in range(time_window):  # simulation time steps
            ...
            first2 = nn.functional.conv2d(event_fea, self.conv1_f2.weight.data, bias=None, stride=2, padding=1, dilation=1, groups=1).detach()
            first3 = nn.functional.conv2d(event_fea, self.conv1_f3.weight.data, bias=None, stride=2, padding=2, dilation=1, groups=1).detach()
            first4 = nn.functional.conv2d(event_fea, self.conv1_f4.weight.data, bias=None, stride=2, padding=3, dilation=1, groups=1).detach()
            first = torch.cat([w1*first2, w2*first3, w3*first4], dim=1)
            first = nn.functional.conv2d(first, self.conv1_cat.weight.data, bias=None, stride=1, padding=0, dilation=1, groups=1).detach()
            c1_mem, c1_spike = mem_update(first, c1_mem, c1_spike)
            second = nn.functional.conv2d(c1_spike, self.conv2_f2.weight.data, bias=None, stride=2, padding=1, dilation=1, groups=1).detach()
            c2_mem, c2_spike = mem_update(second, c2_mem, c2_spike)
            thred = nn.functional.conv2d(c2_spike, self.conv3_f2.weight.data, bias=None, stride=4, padding=1, dilation=1, groups=1).detach()
